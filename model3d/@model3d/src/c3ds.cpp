...
// Description:
//
//   Implementation of class for reading ".3ds" files.  
...
ChunkStruct C3DS::chunkStruct[] = 
{
  ...
  {0xa000,"Material Creation",&C3DS::read_0xa000_chunk},
	{0xa200,"Texture Map",&C3DS::read_0xa200_chunk},
	{0xa300,"Texture Map Name",&C3DS::read_0xa300_chunk},
  ...
  {0x4130,"Apply Material",&C3DS::read_0x4130_chunk},
...
int C3DS::read_0xa300_chunk(byte *s, int len)
{
	strncpy(curMaterial->textureFile,(char *)s,64);
	curMaterial->useTexture = true;
	return len;
}


int C3DS::read_0xa200_chunk(byte *s, int len)
{
	curMaterial->useTexture = true;
	return 0;
}
...
int C3DS::create_layer()
{
  ...
  curLayer->transparency = 0.0f;
	curLayer->useTexture = false;
	curLayer->textureFile[0] = '\0';
...
int C3DS::create_material()
{
  ...
  curMaterial = (Material3DS *)malloc(sizeof(Material3DS));
	curMaterial->useTexture = false;
	curMaterial->textureFile[0] = '\0';
...
int C3DS::apply_material(const char *matname)
{
  ...
  curLayer->transparency = material->transparency;
	curLayer->useTexture = material->useTexture;
	if(curLayer->useTexture) 
		strncpy(curLayer->textureFile,material->textureFile,64);
...
int C3DS::read_0x4130_chunk(byte *head, int len)
{
  ...
  if(nfacet[0] != curLayer->nFacetIdx) {
    ...
    layer->vertices = (Vector *)malloc(sizeof(Vector)*nNewVertices);
		layer->useTexture = curLayer->useTexture;
		strcpy(layer->textureFile,curLayer->textureFile);
